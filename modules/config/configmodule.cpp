...
ModuleHelp ConfigModule::help() const
{
    ...
    result.addUsage(ModuleHelpUsage("Change a config value",
                                    "!config config_name field=value",
                                    "!config modules_board enabled=true"));

    result.addUsage(ModuleHelpUsage("Reset a config value to default",
                                    "!config config_name field reset",
                                    "!config modules_board enabled reset"));

    result.addUsage(ModuleHelpUsage("Change a config default value (Head-admin only)",
                                    "!config config_name default field=value",
                                    "!config modules_board default enabled=true"));

    result.addUsage(ModuleHelpUsage("Change a global config value (Head-admin only)",
        ...
        if (args.size() > 1)
        {
            ...
            if (conf)
            {
                if (args.size() > 3 && args[2].toString().trimmed() == "default")
                {
                    ...
                    response = fSetDefaultConfig(conf, confVals.first, confVals.second);
                }
                else if (args.size() > 3 && args[2].toString().trimmed() == "global")
                {
                    auto confVals = getNameValue(message.getStringFromArgument(3));
                    response = fSetGlobalConfig(conf, confVals.first, confVals.second);
                }
                else if (args.size() > 2)
                {
                    auto confVals = getNameValue(message.getStringFromArgument(2));
                    response = fSetConfig(message.chatId(), conf, confVals.first, confVals.second);
                }
                else
...
QString ConfigModule::fSetConfig(qint64 gid, BotConfig *botConfig, const QString &field, const QVariant &value)
{
...
QString ConfigModule::fSetDefaultConfig(BotConfig *botConfig, const QString &field, const QVariant &value)
{
...
QString ConfigModule::fSetGlobalConfig(BotConfig *botConfig, const QString &field, const QVariant &value)
{
